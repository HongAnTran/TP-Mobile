...
export default function NavigationCategoryClient({ className, data }: { data: CategoryProduct[] | CategoryArtice [], className?: string }) {
  ...
  return (
    <div className={cn(' hidden md:block bg-[#f8f2e6] px-4    text-primary   py-2 transition-all ', isSticky ? ' fixed top-0 left-0 right-0 z-50 bg-white text-primary shadow-2xl' : '', className)}>
          ...
          {data.map(category => {
            return (
              <NavLink scroll absolute href={`${routes.category}/${category.slug}`} key={category.id} activeClassName='text-blue-500    font-bold'>
                <li className='    whitespace-nowrap text-current  text-lg   font-medium  hover:text-blue-500 transition-colors'>
                  {category.title}
